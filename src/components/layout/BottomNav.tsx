      ...
      <div className="flex justify-center p-3 pointer-events-none">
        <div className="pointer-events-auto flex items-center justify-between gap-2 bg-white/80 backdrop-blur-md rounded-full shadow-[0_8px_32px_rgba(0,0,0,0.12)] p-2 w-full max-w-sm">
          ...
          <div className="flex items-center gap-1">
            ...
            <NavLink to="/" end>
              {({ isActive }) => (
                <div className={`flex flex-col items-center justify-center gap-1 rounded-full px-4 h-11 transition-all duration-200 ${isActive ? 'bg-neutral-200' : ''}`}>
                  <LayoutGrid
                    className={`w-5 h-5 shrink-0 transition-colors duration-200 ${isActive ? 'text-neutral-800' : 'text-neutral-600'}`}
                    strokeWidth={2.2}
                  />
                  <span className={`text-[10px] font-medium leading-none transition-colors duration-200 ${isActive ? 'text-neutral-800' : 'text-neutral-600'}`}>
                    Collection
                  </span>
                </div>
              )}
            ...
            <NavLink to="/settings">
              {({ isActive }) => (
                <div className={`flex flex-col items-center justify-center gap-1 rounded-full px-4 h-11 transition-all duration-200 ${isActive ? 'bg-neutral-200' : ''}`}>
                  <Settings
                    className={`w-[22px] h-[22px] shrink-0 transition-colors duration-200 ${isActive ? 'text-neutral-800' : 'text-neutral-600'}`}
                    strokeWidth={2.2}
                  />
                  <span className={`text-[10px] font-medium leading-none transition-colors duration-200 ${isActive ? 'text-neutral-800' : 'text-neutral-600'}`}>
                    Settings
